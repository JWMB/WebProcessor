    ...
    const gridOptions: GridOptions<any> = {
		defaultColDef: {
  		...
  		columnDefs: [
			...
			{ headerName: "Num trainings", field: "summaries", cellRenderer: (params: any) => params.value.length },
			{ headerName: "Num started", field: "summaries", cellRenderer: (params: any) =>
                 xxx(params, ts => ts.filter(t => t.firstLogin != null).length) },
			{ headerName: "Max day", field: "summaries", cellRenderer: (params: any) =>
                 xxx(params, ts => max(ts.map(t => t.trainedDays))) },
			{ headerName: "Min day", field: "summaries", cellRenderer: (params: any) =>
                 xxx(params, ts => min(ts.filter(t => t.firstLogin != null).map(t => t.trainedDays))) },
			{ headerName: "Avg days", field: "summaries", cellRenderer: (params: any) => 
                xxx(params, ts => sum(ts.filter(t => t.firstLogin != null).map(t => t.trainedDays)) / ts.filter(t => t.firstLogin != null).length) },
...
<div>
	<div id="agTrainingGroups" style="height: 200px;" class="ag-theme-alpine"></div>
</div>
