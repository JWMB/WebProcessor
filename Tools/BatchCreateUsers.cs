...
namespace Tools
{
    public class BatchCreateUsers
    {
        ...
        public BatchCreateUsers(IUserRepository userRepository, ITrainingRepository trainingRepository, CreateUserWithTrainings createUserWithTrainings)
        {
        ...
        public async Task CreateAndEmail(string rootPath, IConfiguration config, IEnumerable<string> emails, bool actuallyCreate = false)
        {
            ...
            else
            {
                ...
                createdUsersInfo = (await createUserWithTrainings.CreateUsers(emails, new Dictionary<string, int> { { "Test", 2 } }, "2023 HT template", actuallyCreate)).ToList();
